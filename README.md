...
- **MNIST**: 3 convolutional blocks (2 conv layers per block) with BatchNorm + LeakyReLU; downsampling via stride=2 in blocks 2 and 3 (no MaxPooling); Dropout2d after each block; Global Average Pooling before fully connected layers
- **CIFAR-10**: 3 conv blocks with strided downsampling (no MaxPooling): [3→64, 64→128 (stride 2)] → [128→256, 256→256 (stride 2)] → [256→512, 1×1→256]; Adaptive Avg Pool to (1,1); 3 FC layers (256→192→128→classes) with LeakyReLU and dropout
- Batch normalization throughout; downsampling via strided conv; global/adaptive average pooling before FC
...
3. **Non-IID Data**: Use `--strategy fedprox` for better performance
